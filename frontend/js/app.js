...
//   // ── Coloured segments ────────────────────────────────
//   const segs = [
//       [Math.PI,        Math.PI * 1.16, '#16a34a99'],  // LOW    0–15%
//       [Math.PI * 1.14, Math.PI * 1.61, '#d9770699'],  // MEDIUM 15–60%
//       [Math.PI * 1.59, Math.PI * 2,    '#dc262699'],  // HIGH   60–100%
//   ];
//   segs.forEach(([s, e, c]) => {
//       ctx.beginPath();
//       ctx.arc(cx, cy, r, s, e);
...
function drawGauge(prob, color) {
  ...
  // ── Segments (no background track) ──────────────────
  ctx.beginPath();
  ctx.arc(cx, cy, r, Math.PI, Math.PI * 1.15);
  ctx.strokeStyle = '#16a34a99';
  ctx.lineWidth = 18;
  ctx.lineCap = 'round';
  ctx.stroke();

  ctx.beginPath();
  ctx.arc(cx, cy, r, Math.PI * 1.13, Math.PI * 1.60);
  ctx.strokeStyle = '#d9770699';
  ctx.lineWidth = 18;
  ctx.lineCap = 'round';
  ctx.stroke();

  ctx.beginPath();
  ctx.arc(cx, cy, r, Math.PI * 1.58, Math.PI * 2);
  ...
  factors.slice(0, 10).forEach(f => {
    ...
    div.innerHTML = `
      <div class="shap-factor-top">
        <span class="shap-factor-name">${f.label}</span>
        <div class="d-flex align-items-center gap-2">
          <span class="shap-factor-val">val=${formatVal(f.value)}</span>
          <span class="shap-factor-shap" style="color:${color}">${arrow} ${Math.abs(f.shap_value).toFixed(4)}</span>
        </div>
      </div>
      <div class="shap-bar-track">
